...
////// Lighting:
////// Emissive:
                float3 Primary = _Primary_Color.rgb;
                float3 Secondary = _Secondary_Color.rgb;
                float4 node_933 = _Time;
                float2 node_3715 = ((0.5*i.uv0)+node_933.g*float2(2,1));
                ...
                float G = _Plazma_var.g;
                float R = _Plazma_var.r;
                float node_4942 = 0.5;
                ...
                float node_3333 = 0.5;
                float2 node_3471 = ((node_229+(i.uv0+node_933.g*float2(0.5,1)))*node_3333);
                float4 node_2082 = tex2D(_Center_Noise,TRANSFORM_TEX(node_3471, _Center_Noise));
                float2 node_5242 = ((node_229+(i.uv0+node_933.g*float2(1,0.2)))*node_3333);
                float4 node_2801 = tex2D(_Center_Noise,TRANSFORM_TEX(node_5242, _Center_Noise));
                float node_8150 = (saturate(((node_2082.r*node_2082.r*node_2082.r)+(node_2801.r*node_2801.r*node_2801.r)))*_UV_Distort_var.b*_Dot_Glow);
                float Temp = node_8150;
                float node_7653 = Temp;
                float node_7319 = (G+(R+node_7653)+node_7653);
                float3 emissive = lerp(Primary,Secondary,node_7319);
                float3 finalColor = emissive;
                fixed4 finalRGBA = fixed4(finalColor,node_7319);
